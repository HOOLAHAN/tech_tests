...
describe 'Statement class' do
  ...
  it 'prints the statement line by line' do
    ...
    expect(terminal).to receive(:puts).with("11/12/2022 || || -100 || 400").ordered
    expect(terminal).to receive(:puts).with("10/12/2022 || 300 || || 500").ordered
    expect(terminal).to receive(:puts).with("09/12/2022 || 200 || || 200").ordered
